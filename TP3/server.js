...
app.get("/tasks", async (req, res) => {
  try {
    ...
    const result = status
      ? await pool.query("SELECT * FROM tasks WHERE status = $1 ORDER BY id", [
          status,
        ])
...
// PUT /tasks/:id - Actualizar una tarea existente (RF-04)
app.put("/tasks/:id", async (req, res) => {
  try {
    ...
    const values = [title, description, status, dueDate, id];
...
// DELETE /tasks/:id - Eliminar una tarea (RF-05)
